...
def perform_draw():
    st.session_state.team_milan, st.session_state.team_inter = [], []
    ...
    for position, players in available_players_by_position.items():
        ...
        for i, player in enumerate(players):
            if i % 2 == 0: st.session_state.team_milan.append(player)
            else: st.session_state.team_inter.append(player)

def clear_draw():
    st.session_state.team_milan, st.session_state.team_inter = [], []

c1, c2, _ = st.columns([2, 2, 8])
c1.button("⚡ Sortear Times!", on_click=perform_draw, type="primary", width='stretch')
c2.button("🧹 Limpar Sorteio", on_click=clear_draw, width='stretch')
st.write("---")
