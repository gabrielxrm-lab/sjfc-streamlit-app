...
if not game_stats:
    ...
    # Agrupando os dados por jogador e somando as estatísticas
    ...
    ranking.rename(columns={'player_name': 'Jogador'}, inplace=True)
    ...
    with col1:
        ...
        artilharia_df.index += 1 # Começa o índice do ranking em 1
    ...
    with col4:
        ...
        premios_df = premios_df[
            (premios_df['Craque_do_Jogo'] > 0) | 
            (premios_df['Goleiro_do_Jogo'] > 0) | 
            (premios_df['Gol_do_Jogo'] > 0)
        ].sort_values(by='Craque_do_Jogo', ascending=False).reset_index(drop=True)
        premios_df.index += 1
