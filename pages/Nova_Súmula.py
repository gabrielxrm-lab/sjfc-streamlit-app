...
def montar_sumula_texto(dados_jogo):
    ...
    sumula_partes = [
        ...
        f"⚽ GOL DO JOGO → {dados_jogo.get('gol_do_jogo', '(Não preenchido)')}", f"🧤 GOLEIRO DO JOGO → {dados_jogo.get('goleiro_do_jogo', '(Não preenchido)')}", f"⭐ CRAQUE DO JOGO → {dados_jogo.get('craque_do_jogo', '(Não preenchido)')}", "",
        "________________________________________", f"🔴⚫ Gols do {dados_jogo.get('home_name', 'Casa')}:", "", formatar_gols(sd.get('goals_home', {})), "", f"🟦⬛ Gols do {dados_jogo.get('away_name', 'Visitante')}:", "", formatar_gols(sd.get('goals_away', {})),
        "________________________________________", "", f"🟨 Cartões Amarelos – {dados_jogo.get('data', '')}", ""
    ]
    amarelos = [f"{n} ({dados_jogo.get('home_name', 'Casa')})" for n in sd.get('yellow_cards_home', [])] + [f"{n} ({dados_jogo.get('away_name', 'Visitante')})" for n in sd.get('yellow_cards_away', [])]
    sumula_partes.append("\n".join(amarelos) or "(Sem cartões amarelos)")
    sumula_partes.extend([
        "", f"🟥 Cartões Vermelhos – {dados_jogo.get('data', '')}", "",
        "\n".join([f"{n} ({dados_jogo.get('home_name', 'Casa')})" for n in sd.get('red_cards_home', [])] + [f"{n} ({dados_jogo.get('away_name', 'Visitante')})" for n in sd.get('red_cards_away', [])]) or "(Sem cartões vermelhos)",
...
def save_stats_and_download_sumula():
    sd = st.session_state.sumula_data
    stats_by_player = defaultdict(lambda: {'goals': 0, 'yellow_cards': 0, 'red_cards': 0, 'craque_do_jogo': False, 'goleiro_do_jogo': False, 'gol_do_jogo': False})
...
if not IS_DIRETORIA:
    st.warning("🔒 Apenas a Diretoria pode criar ou editar súmulas.")
...
with col_home:
    # ... (código da coluna Time da Casa, sem alterações, apenas reformatado)
    with st.container(border=True):
...
with col_away:
    # ... (código da coluna Time Visitante, sem alterações, apenas reformatado)
    with st.container(border=True):
