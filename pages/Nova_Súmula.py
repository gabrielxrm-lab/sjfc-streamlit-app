...
import sidebar # Importa a barra lateral

# --- CHAMA A BARRA LATERAL E APLICA O CSS ---
...
def montar_sumula_texto(dados_jogo):
    ...
    sumula_partes = [
        ...
        f"🔴⚫ Gols do {dados_jogo.get('home_name', 'Casa')}:\n\n{formatar_gols(sd.get('goals_home', {}))}",
...
def create_award_section(title, key, placeholder):
    with st.container(border=True):
        ...
        with st.form(f"form_award_{key}", clear_on_submit=True):
            new_item = st.text_input("Adicionar jogador:", key=f"add_award_{key}", placeholder=placeholder, label_visibility="collapsed", disabled=not IS_DIRETORIA)
        ...
        for i, item in enumerate(list(st.session_state.sumula_data.get(key, []))):
            ...
            if c2.button("🗑️", key=f"del_award_{key}_{i}", use_container_width=True, disabled=not IS_DIRETORIA):
                st.session_state.sumula_data[key].pop(i); st.rerun()

c1, c2, c3 = st.columns(3)
with c1: create_award_section("⭐ Craque(s) do Jogo", "craques_do_jogo", "Nome do craque")
with c2: create_award_section("🧤 Goleiro(s) do Jogo", "goleiros_do_jogo", "Nome do goleiro")
with c3: create_award_section("⚽ Gol(s) do Jogo", "gols_do_jogo", "Nome do autor do gol")
...
st.write("---")
with st.container(border=True):
    st.header("📄 Prévia e Download da Súmula"); dados_jogo_dict = {'data': data_jogo.strftime("%d-%m-%Y"), 'dia': dia_semana, 'rodada': rodada, 'gol_do_jogo': None, 'goleiro_do_jogo': None, 'craque_do_jogo': None, 'home_name': home_name, 'away_name': away_name}; sumula_final = montar_sumula_texto(dados_jogo_dict)
