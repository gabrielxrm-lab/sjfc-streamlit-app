...
def save_stats_and_download_sumula(is_pdf=False):
    sd = st.session_state.sumula_data; from collections import defaultdict; stats_by_player = defaultdict(lambda: {'goals': 0, 'yellow_cards': 0, 'red_cards': 0, 'craque_do_jogo': False, 'goleiro_do_jogo': False, 'gol_do_jogo': False})
...
if not IS_DIRETORIA:
    st.warning("🔒 Apenas a Diretoria pode criar ou editar súmulas. Para editar, acesse como Diretoria na página principal.")

if st.button("🗑️ Limpar Campos da Súmula", type="secondary", on_click=limpar_sumula, disabled=not IS_DIRETORIA):
...
with st.container(border=True):
    ...
    gol_jogo = c1.text_input("Gol do Jogo", key="gol_jogo", disabled=not IS_DIRETORIA)
    goleiro_jogo = c2.text_input("Goleiro do Jogo", key="goleiro_jogo", disabled=not IS_DIRETORIA)
    craque_jogo = c3.text_input("Craque do Jogo", key="craque_jogo", disabled=not IS_DIRETORIA)
...
with col_home:
    with st.container(border=True):
        ...
        home_name = st.text_input("Nome", value="SÃO JORGE", key="home_name", disabled=not IS_DIRETORIA)
        ...
        with st.form("home_goal_form", clear_on_submit=True):
            ...
            nome_gol = nome.text_input("Jogador", key="h_g_n", label_visibility="collapsed", placeholder="Nome", disabled=not IS_DIRETORIA)
            camisa_gol = camisa.number_input("Camisa", min_value=1, step=1, key="h_g_c", label_visibility="collapsed", disabled=not IS_DIRETORIA)
            qtd_gol = qtd.number_input("Qtd", min_value=1, step=1, value=1, key="h_g_q", label_visibility="collapsed", disabled=not IS_DIRETORIA)
        ...
        for (nome, camisa), qtd in list(sd.get('goals_home', {}).items()):
            ...
            if c2.button("🗑️", key=f"del_hg_{nome}_{camisa}", use_container_width=True, disabled=not IS_DIRETORIA):
                del sd['goals_home'][(nome, camisa)]
                st.rerun()

# ... (O restante do código segue o mesmo padrão, com a flag 'disabled=not IS_DIRETORIA' em todos os botões e campos de input)
# O código completo está abaixo para garantir
        st.write("---")
        with st.form("home_card_form", clear_on_submit=True):
            st.markdown("**Adicionar Cartão**"); nome_cartao = st.text_input("Jogador", key="h_c_n", label_visibility="collapsed", placeholder="Nome do Jogador", disabled=not IS_DIRETORIA); c1, c2 = st.columns(2)
...
with col_away:
    with st.container(border=True):
        ...
        with st.form("away_card_form", clear_on_submit=True):
            st.markdown("**Adicionar Cartão**"); nome_cartao = st.text_input("Jogador", key="a_c_n", label_visibility="collapsed", placeholder="Nome do Jogador", disabled=not IS_DIRETORIA); c1, c2 = st.columns(2)
            if c1.form_submit_button("🟨 Amarelo", use_container_width=True, disabled=not IS_DIRETORIA):
...
with st.container(border=True):
    ...
    if REPORTLAB_AVAILABLE and c2.button("Salvar Estatísticas e Baixar PDF", use_container_width=True, on_click=save_stats_and_download_sumula, args=(True,), disabled=not IS_DIRETORIA):
