...
def gerar_bolinhas(qtd):
    try: qtd = int(qtd); return " " + "⚽" * min(qtd, 12) if qtd > 0 else ""
...
def montar_sumula_texto(dados_jogo):
    # ... (esta função permanece a mesma, não precisa ser alterada)
...
def save_stats_and_download_sumula(is_pdf=False):
    """Função para processar os dados da súmula, salvar no DB e preparar para download."""
    sd = st.session_state.sumula_data
    stats_by_player = defaultdict(lambda: {
        'goals': 0, 'yellow_cards': 0, 'red_cards': 0,
        'craque_do_jogo': False, 'goleiro_do_jogo': False, 'gol_do_jogo': False
    })

    # Coleta de gols
    for (player_name, _), goals in sd.get('goals_home', {}).items():
        stats_by_player[player_name]['goals'] += goals
    for (player_name, _), goals in sd.get('goals_away', {}).items():
        stats_by_player[player_name]['goals'] += goals
    
    # Coleta de cartões
    for player_name in sd.get('yellow_cards_home', []) + sd.get('yellow_cards_away', []):
        stats_by_player[player_name]['yellow_cards'] += 1
    for player_name in sd.get('red_cards_home', []) + sd.get('red_cards_away', []):
        stats_by_player[player_name]['red_cards'] += 1
    ...
    # Formata a lista para salvar no DB
    final_stats_list = []
    game_date_str = st.session_state.data_jogo.strftime("%Y-%m-%d")
    for player_name, stats in stats_by_player.items():
    ...
    if final_stats_list:
        data_manager.save_game_stats_to_db(final_stats_list)
    
    # Limpa a súmula após salvar
    limpar_sumula()

# ... (o resto do código da interface da súmula permanece o mesmo)
...
with st.container(border=True):
    ...
    c1, c2, c3 = st.columns(3); data_jogo = c1.date_input("Data", format="DD/MM/YYYY", key="data_jogo"); dia_semana = c2.text_input("Dia da Semana", value=data_jogo.strftime("%A")); rodada = c3.text_input("Rodada")
...
col_home, col_away = st.columns(2); sd = st.session_state.sumula_data
with col_home:
    with st.container(border=True):
        ...
        for (nome, camisa), qtd in list(sd.get('goals_home', {}).items()):
            ...
            if c2.button("🗑️", key=f"del_hg_{nome}_{camisa}", use_container_width=True): del sd['goals_home'][(nome, camisa)]; st.rerun()
        st.write("---")
        with st.form("home_card_form", clear_on_submit=True):
            st.markdown("**Adicionar Cartão**"); nome_cartao = st.text_input("Jogador", key="h_c_n", label_visibility="collapsed", placeholder="Nome do Jogador"); c1, c2 = st.columns(2)
        ...
        for i, nome in enumerate(list(sd.get('yellow_cards_home', []))):
            ...
            if c2.button("🗑️", key=f"del_hy_{nome}_{i}", use_container_width=True): sd['yellow_cards_home'].pop(i); st.rerun()
        for i, nome in enumerate(list(sd.get('red_cards_home', []))):
            c1, c2 = st.columns([4, 1]); c1.text(f"🟥 {nome}");
            if c2.button("🗑️", key=f"del_hr_{nome}_{i}", use_container_width=True): sd['red_cards_home'].pop(i); st.rerun()
with col_away:
    with st.container(border=True):
        ...
        for (nome, camisa), qtd in list(sd.get('goals_away', {}).items()):
            c1, c2 = st.columns([4, 1]); c1.text(f"⚽ {nome} ({camisa}) - {qtd} gol(s)");
            if c2.button("🗑️", key=f"del_ag_{nome}_{camisa}", use_container_width=True): del sd['goals_away'][(nome, camisa)]; st.rerun()
        st.write("---")
        with st.form("away_card_form", clear_on_submit=True):
            st.markdown("**Adicionar Cartão**"); nome_cartao = st.text_input("Jogador", key="a_c_n", label_visibility="collapsed", placeholder="Nome do Jogador"); c1, c2 = st.columns(2)
            if c1.form_submit_button("🟨 Amarelo", use_container_width=True):
        ...
        for i, nome in enumerate(list(sd.get('yellow_cards_away', []))):
            c1, c2 = st.columns([4, 1]); c1.text(f"🟨 {nome}");
            if c2.button("🗑️", key=f"del_ay_{nome}_{i}", use_container_width=True): sd['yellow_cards_away'].pop(i); st.rerun()
        for i, nome in enumerate(list(sd.get('red_cards_away', []))):
            c1, c2 = st.columns([4, 1]); c1.text(f"🟥 {nome}");
            if c2.button("🗑️", key=f"del_ar_{nome}_{i}", use_container_width=True): sd['red_cards_away'].pop(i); st.rerun()
...
def create_occurrence_section(title, key, placeholder):
    with st.container(border=True):
        ...
        for i, item in enumerate(list(sd.get(key, []))):
            ...
            if c2.button("🗑️", key=f"del_{key}_{i}", use_container_width=True): sd[key].pop(i); st.rerun()
...
with st.container(border=True):
    ...
    dados_jogo_dict = {'data': data_jogo.strftime("%d-%m-%Y"), 'dia': dia_semana, 'rodada': rodada, 'gol_do_jogo': gol_jogo, 'goleiro_do_jogo': goleiro_jogo, 'craque_do_jogo': craque_jogo, 'home_name': home_name, 'away_name': away_name}
    ...
    c1, c2 = st.columns(2)
    if c1.button("Salvar Estatísticas e Baixar TXT", use_container_width=True, type="primary", on_click=save_stats_and_download_sumula):
    ...
    if REPORTLAB_AVAILABLE and c2.button("Salvar Estatísticas e Baixar PDF", use_container_width=True, on_click=save_stats_and_download_sumula, args=(True,)):
