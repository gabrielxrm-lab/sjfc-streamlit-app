...
import streamlit as st; import pandas as pd; import data_manager; from datetime import datetime; import os
...
with c1: st.title("Gerenciamento de Jogadores")
with c2:
...
if not IS_DIRETORIA: st.warning("🔒 Modo de visualização. Para editar, acesse como Diretoria na página principal.")
if IS_DIRETORIA:
    with st.expander("➕ Cadastrar Novo Jogador ou Editar Existente"):
        ...
        with st.form("player_form", clear_on_submit=True):
            name = st.text_input("Nome", value=player_to_edit['name'] if player_to_edit else ""); position_list = ["GOLEIRO", "ZAGUEIRO", "LATERAL", "MEIO-CAMPO", "ATACANTE"]; pos_index = position_list.index(player_to_edit['position']) if player_to_edit and player_to_edit.get('position') in position_list else 3; position = st.selectbox("Posição", position_list, index=pos_index); dob = st.text_input("Data Nasc.", value=player_to_edit.get('date_of_birth', '') if player_to_edit else ""); phone = st.text_input("Telefone", value=player_to_edit.get('phone', '') if player_to_edit else "")
            uploaded_photo = st.file_uploader("Alterar Foto", type=['png', 'jpg', 'jpeg']); current_photo_identifier = player_to_edit.get('photo_file', '') if player_to_edit else ''
            if current_photo_identifier and current_photo_identifier.startswith('http'): st.caption("Foto atual:"); st.image(current_photo_identifier, width=100)
            ...
            if submitted:
                if not name or not position: st.error("Nome e Posição são obrigatórios.")
                    ...
                    if uploaded_photo is not None: file_bytes = uploaded_photo.getvalue(); file_path_in_bucket = f"fotos_jogadores/{name.replace(' ', '-').lower()}.{uploaded_photo.name.split('.')[-1]}"; photo_identifier = data_manager.upload_file_to_storage(file_bytes, file_path_in_bucket)
                    ...
                    st.info("Lembre-se de salvar na nuvem."); st.rerun()
...
if not df_players.empty:
    ...
    st.write("---"); st.header("🔎 Ficha Detalhada")
    player_to_view_name = st.selectbox("Selecione um jogador", options=[""] + df_players['name'].tolist(), index=0, placeholder="Escolha um jogador...")
