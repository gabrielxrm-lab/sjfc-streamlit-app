...
if not df_players.empty:
    ...
    # --- Seção de Exclusão (CORRIGIDA) ---
    if IS_DIRETORIA:
        st.write("---")
        st.header("🗑️ Excluir Jogadores")
        players_to_delete_names = st.multiselect(
            "Selecione os jogadores para excluir da lista", 
            df_players['name'].tolist()
        )
        
        # Este botão agora está no lugar correto e sempre visível para a diretoria
        if st.button("Remover Selecionados da Lista", type="secondary"):
            if players_to_delete_names:
                ...
                st.toast("Nenhum jogador selecionado para exclusão.")
else:
