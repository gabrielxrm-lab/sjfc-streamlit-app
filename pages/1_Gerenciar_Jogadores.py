...
with st.expander("➕ Cadastrar Novo Jogador ou Editar Existente", expanded=False):
    # ... (código do formulário, com a lógica de upload alterada)
    ...
    with st.form("player_form", clear_on_submit=True):
        ...
        current_photo_url = player_to_edit.get('photo_file', '') if player_to_edit else ''
        if current_photo_url:
            st.caption("Foto atual:")
            st.image(current_photo_url, width=100)
        ...
        if submitted:
            if not name or not position: st.error("Nome e Posição são obrigatórios.")
                ...
                if uploaded_photo is not None:
                    ...
                    # Cria um caminho único para o arquivo, ex: "fotos_jogadores/nome-do-jogador.jpg"
                    file_path_in_bucket = f"fotos_jogadores/{name.replace(' ', '-').lower()}.{uploaded_photo.name.split('.')[-1]}"
                    photo_url = data_manager.upload_file_to_storage(file_bytes, file_path_in_bucket)
                
                player_data = {
                    'name': name.upper(), 'position': position, 'date_of_birth': dob, 
                    'phone': phone, 'photo_file': photo_url
                }
...
if not df_players.empty:
    ...
    # ... (lógica da ficha, agora usando a URL da imagem)
    ...
    if player_to_view_name:
        ...
        with st.container(border=True):
            ...
            with col_img:
                photo_url = player_data.get('photo_file', '')
                if photo_url:
    ...
    # --- Seção de Exclusão ---
    # ... (código da exclusão permanece o mesmo)
