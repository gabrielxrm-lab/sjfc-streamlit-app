  ...
  return (
    ...
    >
      <header className="text-center space-y-4">
        ...
        <div className="inline-block px-4 py-2 rounded-full bg-zinc-900 border border-zinc-800 text-sm font-medium mt-4">
      ...
      <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center">
        <h2 className="text-zinc-400 uppercase tracking-widest text-sm font-bold mb-4 flex items-center justify-center gap-2">
          <Calendar size={18} />
          Próximo Jogo: Domingo, 07:00
        </h2>
        <div className="font-mono text-4xl md:text-6xl font-bold text-indigo-400 tracking-wider">
          {timeLeft || 'Calculando...'}
        </div>
      </section>
      ...
      <section>
        <h2 className="text-2xl font-bold mb-6 flex items-center gap-3">
          <span>🎂</span> Aniversariantes de {monthNames[currentMonth - 1]}
        </h2>
        ...
        {birthdays.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {birthdays.map(player => {
              ...
              return (
                <div key={player.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden group hover:border-indigo-500/50 transition-colors">
                  <div className="aspect-square bg-zinc-800 relative">
                    <img src={photoUrl} alt={player.name} className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 to-transparent opacity-80" />
                    <div className="absolute bottom-4 left-4 right-4">
                      <h3 className="font-bold text-lg truncate">{player.name}</h3>
                      {player.shirt_number && <p className="text-zinc-400 text-sm">Camisa: {player.shirt_number}</p>}
                    </div>
                  </div>
                  <div className="p-4 flex items-center justify-between bg-zinc-900">
                    <span className="text-zinc-500 text-sm uppercase tracking-wider font-semibold">Dia</span>
                    <span className="text-3xl font-black text-indigo-400">{day}</span>
                  </div>
                </div>
              );
            })}
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={i} className="aspect-video bg-zinc-800 rounded-xl overflow-hidden">
              <img 
